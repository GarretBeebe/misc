package main; 
import ( "fmt"; "html"; "log"; "net/http"; "strconv"; "strings"; "time" ); 
type ControlMessage 
	...
	func main() 
		{ 
			...
			for { 
					select { 
								case respChan := <- statusPollChannel: respChan <- workerActive; 
								case msg := <-controlChannel: workerActive = true; go doStuff(msg, workerCompleteChan); 
								case status := <- workerCompleteChan: workerActive = status; 
							}
				}
		}; 
	...
	func admin(cc chan ControlMessage, statusPollChannel chan chan bool) 
		{
			...
			http.HandleFunc("/status",
				...
				{ 
					...
					select { 
								case result := <- reqChan: if result { fmt.Fprint(w, "ACTIVE"); } else { fmt.Fprint(w, "INACTIVE"); }; return; 
								case <- timeout: fmt.Fprint(w, "TIMEOUT");
							}
				}
			); 
